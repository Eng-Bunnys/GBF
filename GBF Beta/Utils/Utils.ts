...
/**
 * Converts a 24-hour time into a 12-hour formatted time with AM/PM.
 *
 * This function supports input in the following formats:
 * - Integers like 1446 for 14:46
 * - Decimal numbers like 14.46 for 14:46
 * - Strings like "0900" for 09:00
 *
 * @param {number | string} Time24 - The 24-hour time to convert.
 * @returns {string} The converted 12-hour formatted time with AM/PM.
 *
 * @example
 * TwentyFourToTwelveHour(1446) // Returns "2:46 PM"
 * TwentyFourToTwelveHour("0900") // Returns "9:00 AM"
 * TwentyFourToTwelveHour(14.46) // Returns "2:46 PM"
 * TwentyFourToTwelveHour(0) // Returns "12:00 AM"
 */
export function TwentyFourToTwelveHour(Time24: number | string): string {
  ...
  if (typeof Time24 === "number") {
    if (Time24 % 1 !== 0) {
      const [IntegerSection, FractionPart] = Time24.toString().split(".");

      ConvertedHours = parseInt(IntegerSection, 10);

      ConvertedMinutes = parseFloat(`0.${FractionPart}`) * 100;
    } else {
      ConvertedTime = Time24.toString().padStart(4, "0");
      ConvertedHours = parseInt(ConvertedTime.slice(0, 2), 10);
      ConvertedMinutes = parseInt(ConvertedTime.slice(2, 4), 10);
    }
  } else if (typeof Time24 === "string") {
    ConvertedTime = Time24.padStart(4, "0");
    ConvertedHours = parseInt(ConvertedTime.slice(0, 2), 10);
    ConvertedMinutes = parseInt(ConvertedTime.slice(2, 4), 10);
  } else {
  ...
  const TimePeriod = ConvertedHours < 12 ? "AM" : "PM";

  const TwelveHour = ConvertedHours % 12 === 0 ? 12 : ConvertedHours % 12;

  return `${TwelveHour}:${ConvertedMinutes.toString().padStart(
    2,
    "0"
  )} ${TimePeriod}`;
}
...
/**
 * Splits an array into chunks of a specified size and returns the sum of elements in each chunk.
 * 
 * @param Array - The array to be chunked.
 * @param ChunkSize - The desired chunk size.
 * @returns An array of sums of each chunk.
 */
export function ChunkSum(Array: number[], ChunkSize: number): number[] {
  if (ChunkSize <= 0) throw new Error("Chunk size must be greater than zero.");

  const ResultChunk: number[] = [];
...
/**
 * Returns the sum of a specified number of elements from the front of an array.
 *
 * @param Array - An array of numbers to sum.
 * @param NumberOfElements - The number of elements from the front to sum.
 * @returns The sum of the specified number of elements.
 */
export function SumOfFirstNElements(
  Array: number[],
  NumberOfElements: number
): number {
  if (NumberOfElements <= 0) return 0;

  const SlicedArray = Array.slice(0, NumberOfElements);
