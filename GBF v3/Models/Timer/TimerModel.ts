...
const semesterSchema = z.object({
  ...
  breakCount: z.number().int().min(0, "Break count cannot be negative"),
});
...
const sessionSchema = z.object({
  ...
  lastSessionTopic: z
    .string()
    .min(1, "Last session topic is required")
    .max(200, "Last session topic too long"),
...
const MongoSemesterSchema = new Schema<Semester>(
  {
    ...
    breakCount: {
      type: Number,
      default: 0,
      min: [0, "Break count cannot be negative"],
    },
  },
  { _id: false }
);
