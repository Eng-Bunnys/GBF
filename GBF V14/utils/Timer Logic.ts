 ...
 * @returns {(Array<number | string>)} - An array representing the collection day, rewarded cash, and rewarded coins respectively
 * @returns {Array<number | string>[0]} - The collection day (1 to 7)
 * @returns {Array<number | string>[1]} - The rewarded cash (up to ₲20,000)
...
export function loginReward(streak: number = 1): Array<number | string> {
  ...
  // XP reward
  let CashReward: number = streak * 200;
  if (CashReward > 20000) CashReward = 20000;
  ...
  if (day === 6) CashReward *= 2;
  if (day === 7) coinsReward *= 2;

  if (day !== 7 && day !== 4) coinsReward = 0;
  if (day === 7 || day === 4) CashReward = 0;

  return [day, Math.floor(CashReward), Math.floor(coinsReward)];
}
