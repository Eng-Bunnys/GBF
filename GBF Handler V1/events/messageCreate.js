  ...
  client.on("messageCreate", async (message) => {
    ...
    if (message.content.startsWith(prefix)) {
      const botReply = await message.reply({
        content: `Legacy (prefix) commnads have been disabled globally, please use \`slash commands\` [/]`,
      });
      setTimeout(() => {
        return botReply.delete();
      }, 5000);
    }
    ...
    if (command.Partner && !Partners.includes(message.author.id)) {
      ...
      if (
        partnerchannelony &&
        partnerchannelony.viewable &&
        partnerchannelony
          .permissionsFor(message.guild.me)
          .has(Permissions.FLAGS.SEND_MESSAGES)
      ) {
        return partnerchannelony
          .send({
            embeds: [PartnerOnly],
          })
          .catch(() => {});
    ...
    const commandranembed = new MessageEmbed()
      ...
      .setColor("#e91e63")
      .setDescription(
        `The command **${command.name}** has been ran by **${message.author.tag}** in **${guild.name}** at **${time}, ${date}**`
      );
    ...
    try {
      ...
      const errorembed = new MessageEmbed()
        ...
        .setDescription(
          `> Here is the full error, it has already been reported to my developers:\n \`\`\`js\n${err}\`\`\`\n`
        )
        .setColor("#ff0015");
      const errorchannel = message.channel;
      if (
        errorchannel &&
        errorchannel.viewable &&
        errorchannel
          .permissionsFor(message.guild.me)
          .has(Permissions.FLAGS.SEND_MESSAGES)
      ) {
