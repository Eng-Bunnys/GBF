...
class GBFClient extends Client {
  ...
  async loadCommands() {
    ...
    if (guildCommands.length) {
      const TestServerMain = await this.guilds.fetch("827589582932410388");
      const MonkeJungle = await this.guilds.fetch("439890528583286784");
      const GBFEmotes = await this.guilds.fetch("614515078997475333");
      const SethServ = await this.guilds.fetch("954680527112265749")
      //guildCommands
      await TestServerMain.commands.set(guildCommands);
      await MonkeJungle.commands.set(guildCommands);
      await GBFEmotes.commands.set(guildCommands);
      await SethServ.commands.set(guildCommands);
    }//globalCommands
