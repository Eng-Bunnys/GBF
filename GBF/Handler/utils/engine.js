    ...
    for (const argument of expectedArgs) {
        ...
        for (let i = 0; i < amount; i++) {
            ...
            switch (argument.type) {
                case "SOMETHING":
                    if (argument.words && !argument.words.includes(msgArgs[counter].toLowerCase())) return {
                        invalid: true,
                        prompt: argument.prompt
                    };
                    else if (argument.regexp && !argument.regexp.test(msgArgs[counter])) return {
                        invalid: true,
                        prompt: argument.prompt
                    };
                ...
                case "NUMBER":
                    ...
                    if (argument.min && argument.min > num) return {
                        invalid: true,
                        prompt: argument.prompt
                    };

                    if (argument.max && argument.max < num) return {
                        invalid: true,
                        prompt: argument.prompt
                    };
                ...
                case "CHANNEL":
                    ...
                    if (argument.channelTypes && !argument.channelTypes.includes(channel.type)) return {
                        invalid: true,
                        prompt: argument.prompt
                    };
                ...
                case "ROLE":
                    ...
                    if (argument.notBot && role.managed) return {
                        invalid: true,
                        prompt: argument.prompt
                    };
                ...
                case "MEMBER":
                    ...
                    if (!member) return {
                        ...
                        if (argument.notBot && member.user.bot) return {
                            invalid: true,
                            prompt: argument.prompt
                        };

                        if (argument.notSelf && member.id === message.author.id) return {
                            invalid: true,
                            prompt: argument.prompt
                        };
                ...
                case "TIME":
                    ...
                    if (argument.min && num < argument.min) return {
                        invalid: true,
                        prompt: argument.prompt
                    };

                    if (argument.max && num > argument.max) return {
                        invalid: true,
                        prompt: argument.prompt
                    };
...
function msToTime(time, options = {}) {
    if (options.format === undefined || (options.format !== 'short' && options.format !== 'medium' && options.format !== 'long')) options.format = 'long';
...
function missingPermissions(member, permission) {
    const PermsMissing = member.permissions.missing(permission).map(str => `\`${str.replace(/_/g, ' ').toLowerCase().replace(/\b(\w)/g, char => char.toUpperCase())}\``);

    return PermsMissing.length > 1 ? `${PermsMissing.slice(0, -1).join(", ")}, ${PermsMissing.slice(-1)[0]}` : PermsMissing[0];
}
...
function rolePermissions(role) {
    const rolePerms = role.permissions.toArray().map(str => `${str.replace(/_/g, ' ').toLowerCase().replace(/\b(\w)/g, char => char.toUpperCase())}`)

    return rolePerms.length > 1 ? `${rolePerms.slice(0, -1).join(", ")} ${rolePerms.slice(-1)[0]}` :
        rolePerms[0];
}
...
function KeyPerms(role) {
    ...
    if (role.permissions.has(Permissions.FLAGS.CHANGE_NICKNAME)) KeyPermissions.push(`Change Nickname`)
    if (role.permissions.has(Permissions.FLAGS.MANAGE_EMOJIS_AND_STICKERS)) KeyPermissions.push(`Manage Emojis & Stickers`)
    if (role.permissions.has(Permissions.FLAGS.MANAGE_WEBHOOKS)) KeyPermissions.push(`Manage Webhooks`)
    ...
    if (role.permissions.has(Permissions.FLAGS.USE_EXTERNAL_EMOJIS)) KeyPermissions.push(`Use External Emojis`)
    if (role.permissions.has(Permissions.FLAGS.ADD_REACTIONS)) KeyPermissions.push(`Add Reactions`)
    if (role.permissions.has(Permissions.FLAGS.MUTE_MEMBERS)) KeyPermissions.push(`Mute Members`)
    if (role.permissions.has(Permissions.FLAGS.DEAFEN_MEMBERS)) KeyPermissions.push(`Deafen Members`)
    if (role.permissions.has(Permissions.FLAGS.MOVE_MEMBERS)) KeyPermissions.push(`Move Members`)
    if (role.permissions.has(Permissions.FLAGS.MODERATE_MEMBERS)) KeyPermissions.push(`Moderate Members`)
    if (role.permissions.has(Permissions.FLAGS.VIEW_AUDIT_LOG)) KeyPermissions.push(`View Audit Log`)
    if (role.permissions.has('SEND_MESSAGES')) KeyPermissions.push(`Send Messages`)
    if (role.permissions.has('ATTACH_FILES')) KeyPermissions.push(`Attach Files`)
    if (role.permissions.has('READ_MESSAGE_HISTORY')) KeyPermissions.push(`Read Message History`)
    if (role.permissions.has('CREATE_INSTANT_INVITE')) KeyPermissions.push(`Create Instant Invite`)
...
function roleInGuildCheck(roleIds) {
    ...
    return roleIds.length > 1 ? `<@&${roleIds.slice(0, -1).join(", ")}> <@&${roleIds.slice(-1)[0]}>` :
        roleIds[0];
}
...
function NeededRP(level) {
    return (level * 800) + ((level - 1) * 400);
}

function rewardedCash(level) {
    return level * 5 * 100;
}

function rankAchievments(rank) {
    let RPEmoji
    if (rank <= 100) RPEmoji = `🥉`
    else if (rank <= 500 && rank > 100) RPEmoji = `🥈`
    else if (rank <= 1000 && rank > 500) RPEmoji = `🥇`
    else if (rank <= 5000 && rank > 1000) RPEmoji = `💎`
    return RPEmoji
}

function checkRank(rank, rp, neededRp) {
    if (rank >= 5000) return;
    if (rp >= neededRp) {
        rank += 1;
        neededRp = NeededRP(rank);
        rp = 0;
    } 
}
